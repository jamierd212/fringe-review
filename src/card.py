...
def caption(conn: sqlite3.Connection, placed, when: date | None = None) -> str:
    ...
    A companion file of handles for Instagram's "Tag people" used to be written
    beside this. Photo tagging turned out to be more effort than it returned, so
    it is gone; the mentions below are pasted with the caption and do the same
    job in one step.
    """
    when = when or date.today()
    handles = {
